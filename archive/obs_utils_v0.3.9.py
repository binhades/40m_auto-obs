...
# --- VERSION CONTROL ---
UTILS_VERSION = "v0.3.9 (Strict Schema)"
DATA_VERSION = "v0.1.0"
...
# NEW: The definitive list of allowed keys. Anything else triggers a schema error.
ALLOWED_FIELDS = [
    "project_id", "observer", "source", "ra", "dec", 
    "receiver", "mode", "start_time_cst", "duration", "antennas",
...
def verify_schedule(data):
    ...
    try:
        ...
        for idx, task in enumerate(data["schedule"]):
            ...
            # --- 1. DATA SANITIZATION ---
            for field in ["project_id", "observer", "source", "ra", "dec", "receiver", "mode", "start_time_cst", "duration"]:
            ...
            if unexpected_keys:
                add_error(idx, f"Unexpected/Unknown keys found: {', '.join(unexpected_keys)}. Strict schema enforced.")
