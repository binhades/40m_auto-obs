...
def main_page():
    ...
    def reset_view_only():
        if status_label: status_label.set_text("")
        if status_card: status_card.classes(remove='bg-green-100 bg-orange-100 bg-red-100 bg-blue-100')
        if status_icon: 
            status_icon.name = ''
    ...
    def validate_and_update_ui(data, filename):
        if preview_table is None: return

        # 1. Run Verification
        is_valid, validation_result = obs_utils.verify_schedule(data)
        ...
        for idx, task in enumerate(data.get("schedule", [])):
            ...
            if is_valid:
                ...
                if isinstance(validation_result, dict) and idx in validation_result:
                    task['_status'] = "ERROR"
                    task['_msg'] = " | ".join(validation_result[idx])
        ...
        if upload_area: upload_area.visible = False
        if preview_container: preview_container.visible = True
        ...
        if is_valid:
            ...
            if isinstance(validation_result, dict) and 'global' in validation_result:
                global_msg = " | ".join(validation_result['global'])
                status_label.set_text(f"VALIDATION FAILED: {global_msg}")
            else:
                err_count = len(validation_result) if isinstance(validation_result, dict) else 1
                status_label.set_text(f"VALIDATION FAILED: Found errors in {err_count} task(s). Please review below.")
    ...
    def full_reset():
        state['data'] = None
        if preview_table: preview_table.rows = []
        if upload_area: upload_area.visible = True
        if preview_container: preview_container.visible = False

    # --- LAYOUT ---
    with ui.card().classes('w-full max-w-7xl mx-auto p-6 mt-10'):
        
        # --- NEW UI HEADER SYNCED WITH SCHEDULER ---
        with ui.row().classes('w-full items-center gap-3 mb-6'):
            ui.icon('cloud_upload', size='lg').classes('text-blue-600')
            ui.label("FAST Core Array Schedule Uploader").classes('text-3xl text-black')
            ui.label(VERSION).classes('text-sm text-gray-400 font-mono bg-gray-100 px-2 py-1 rounded')
        # ---------------------------------------------
