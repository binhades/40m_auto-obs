    ...
    def read(self):
        ...
        try:
            ...
            if self.file is None or (self.inode and self.inode != current_inode):
                ...
                # Rewind 20KB for driver history to show context
...
class RemoteLogReader:
    ...
    async def start_loop(self):
        ...
        while self.should_run:
            try:
                # Use strict SSH options to fail fast if connection drops
                cmd = [
...
def check_existing_process():
    """Checks if the Python Driver (run_auto_obs.py) is running."""
    try:
...
def main_page():
    ...
    with ui.tab_panels(tabs, value=tab_sched).classes('w-full h-full'):
        with ui.tab_panel(tab_sched):
            ...
            local_reader = LogReader(obs_utils.DRIVER_LOG_FILE, driver_log)
