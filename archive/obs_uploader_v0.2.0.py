...
VERSION = "v0.2.0 (Visual Feedback)"
...
def main_page():
    ...
    async def handle_upload(e):
        try:
            # File Reading Logic (Compatible with NiceGUI upload events)
            content = ""
            filename = "unknown.json"
            if hasattr(e, 'file'): # Drag and drop
                filename = getattr(e.file, 'name', 'unknown.json')
                content = e.content.read().decode('utf-8')
            elif hasattr(e, 'content'): # Binary upload
