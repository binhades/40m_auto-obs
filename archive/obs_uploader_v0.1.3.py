...
# --- FIX: Must be async to handle file reading ---
async def handle_upload(e):
    """Parses the uploaded file and runs validation."""
    global current_data, current_filename
    
    try:
        # --- 1. Parse JSON (Async Fix for NiceGUI v3.6+) ---
        if hasattr(e, 'file'):
            f = e.file
            # We must AWAIT the read because it is a coroutine
            if hasattr(f, 'read'):
                # Check if read is a coroutine (async) or normal method
                file_content = f.read()
                # If it's a coroutine, await it. If it's bytes, use it.
                ...
                raise ValueError("Cannot read file content from UploadEvent")
        else:
...
def save_to_server():
    ...
    try:
        ...
        # Write to disk (Strip helper field first)
