...
def handle_upload(e):
    ...
    try:
        # --- 1. Parse JSON (Fixed for NiceGUI v3.6.1) ---
        # The event object 'e' uses 'file' instead of 'content' in this version.
        if hasattr(e, 'file'):
            f = e.file
            # Try standard read() first, fallback to direct data access
            if hasattr(f, 'read'):
        ...
        if is_valid:
            # --- FIX: Pre-calculate backend string (Avoids JSON error) ---
