...
def main_page():
    ...
    def handle_upload(e):
        try:
            # FIX: Simplified File Reading Logic
            # NiceGUI passes content as a file-like object in e.content
            filename = e.name
            
            # Read directly from the spool/buffer
            ...
            # Fallback for older NiceGUI versions where e.content might be raw bytes
            try:
                content_str = e.content.decode('utf-8')
                data = json.loads(content_str)
                validate_and_update_ui(data, e.name)
            except:
                ui.notify("Error reading upload content", type='negative')
