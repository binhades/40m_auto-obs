...
# --- STATE MANAGEMENT ---
# We store the uploaded data in memory here.
# It is NOT saved to disk until the user clicks "Save to Server".
current_data = None
...
def handle_upload(e):
    ...
    try:
        ...
        if is_valid:
            ...
            # Check for warnings (Rule 6 in utils)
            ...
            # Invalid
            current_data = None # Prevent saving
            ...
            preview_table.rows = [] # Hide invalid data to avoid confusion
            save_btn.disable()
    ...
    with ui.card().classes('w-full max-w-4xl mx-auto p-6 mt-10'):
        ...
        with ui.column().classes('w-full gap-4 hidden') as preview_container:
            ...
            columns = [
                ...
                {'name': 'backend', 'label': 'Backend', 'field': lambda r: "BB" if r.get('baseband_enabled') else ("Spec" if r.get('spec_enabled') else "PSR"), 'align': 'left'},
            ]
