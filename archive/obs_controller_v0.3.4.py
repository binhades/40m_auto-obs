...
class LogReader:
    def __init__(self, filepath, ui_log_element, parse_status=False, status_callback=None):
        ...
        # Regex patterns to match run_auto_obs.py output
    ...
    def read(self):
        ...
        try:
            ...
            if self.file is None or (self.inode and self.inode != current_inode):
                ...
                # Jump to end for master log to avoid loading history
...
class RemoteLogReader:
    ...
    async def start_loop(self):
        ...
        while self.should_run:
            try:
                # SSH tail -F to stream logs from workers
                cmd = [
...
def main_page():
    ...
    def update_table():
        """Re-renders table rows based on status."""
    ...
    def toggle_skip(source_name):
        # Requirement 2: Lock-down check (Server Side)
        if global_state.running:
    ...
    async def run_observation():
        ...
        try:
            ...
            # Create Temp Schedule for Driver
    ...
    with ui.tab_panels(tabs, value=tab_sched).classes('w-full h-full'):
        with ui.tab_panel(tab_sched):
            ...
            cols = [
                ...
                {'name': 'backends', 'label': 'Backends', 'field': 'backend_display', 'align': 'left'}, # Backend Column
            ...
            # Custom Slot for Colors & Disable Logic
            ...
            # Start Log Reader with Status Callback
