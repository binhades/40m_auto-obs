...
# Version: v0.1.2
# Updates: Per-Antenna Logging (active_spec_CA01.log)
# ==================================================================
...
# --- 3. LOG SETUP (UPDATED FOR PER-ANTENNA) ---
# Usage: setup_log "CA01" "spec"
setup_log() {
    local ant=$1
    local type=$2
    # NEW FORMAT: active_spec_CA01.log
    ...
    # Reset the active log (wipe previous run data for this antenna)
    rm -f "$active_file"
...
# --- EXECUTION FUNCTIONS (UPDATED REDIRECTION) ---
...
run_mbspec() {
    ...
    local nchan_wb=0; local nchan_nb=0; local nb_offs=0; local gpuid=0
    ...
    echo "[WORKER] Launching mbspec for $ant..." >> "$active_log"
...
run_specrecv() {
    ...
    echo "[WORKER] Launching specrecv ($psr_mode) for $ant..." >> "$active_log"
...
run_bbrec() {
    ...
    echo "[WORKER] Launching bbrec for $ant..." >> "$active_log"
...
# Log Setup Loop (Must be done before launching so logs exist)
