...
# Version: v0.2.2
# Updates: Aggressive Static Cleanup (Fixes Ctrl+C issue)
# ==================================================================
...
cleanup() {
    ...
    for host in "${WORKER_NODES[@]}"; do
        ...
        # We explicitly kill the binaries (-9) to ensure immediate stop
...
for (( i=0; i<$task_count; i++ )); do
    ...
    source=$(jq -r ".schedule[$i].source" "$JSON_FILE")
    ...
    echo "---------------------------------------------------"
    echo "Task $((i+1)): $source | Start: $start_time (CST)"
    ...
    if [ $remaining_sec -lt 10 ]; then
        echo "[WARNING] Start time is too close (<10s). Skipping."
        continue
    fi
    ...
    launch_worker() {
        ...
        local cmd="$WORKER_SCRIPT -s \"$source\" -t \"$start_time\" -d $duration -a \"$ants\" -B \"$backends_flag\" $meta_args --psr_mode \"$psr_mode\" --spec_mode \"$spec_mode\""
