...
# --- CONSTANTS ---
DRIVER_LOG_FILE = obs_utils.LOG_DIR / "driver_master.log"
...
# --- HELPER 1: LOCAL Log Streaming ---
class LogReader:
    def __init__(self, filepath, ui_log_element):
        ...
        try:
            ...
            if self.file is None or (self.inode and self.inode != current_inode):
                ...
                if "driver_master" in self.filepath.name:
                    self.file.seek(0, 2) 
                    size = self.file.tell()
                    self.file.seek(max(0, size - 20000)) 
                else:
                    self.file.seek(0)

            lines = self.file.read()
            if lines: self.ui_log.push(lines)
        except Exception: pass 

# --- HELPER 2: REMOTE Log Streaming ---
        ...
        while self.should_run:
            try:
                ...
                while self.process and self.process.returncode is None:
                    ...
                    if line: self.ui_log.push(line.decode().strip())
                    else: break
...
def check_existing_process():
    """Checks if run_auto_obs.sh is truly running (ignores editors)."""
    try:
        # -a lists the full command line
        cmd = ["pgrep", "-a", "-f", "run_auto_obs.sh"]
        output = subprocess.check_output(cmd).decode().strip()
        
        real_process_found = False
        if output:
            for line in output.split('\n'):
                # FILTER OUT EDITORS AND LOG VIEWERS
                if any(x in line for x in ["vi ", "vim ", "nano ", "tail ", "grep ", "cat "]):
                    continue
                
                # Check for actual execution
                if "bash" in line or "./run_auto_obs.sh" in line:
        ...
        if real_process_found:
            if not global_state.running:
                # print(f"Adopting process: {output}") # Debug
        ...
        # pgrep returns non-zero if nothing found
        if global_state.running:
            global_state.running = False
...
def main_page():
    ...
    # UI Refs
    schedule_table = None; driver_log = None; btn_run = None; btn_stop = None; status_label = None
    ...
    def update_table():
        if local_state['schedule_data']:
            schedule_table.rows = local_state['schedule_data']['schedule']
        else:
            schedule_table.rows = []

    def update_status_ui():
        check_existing_process() # Run the new strict check
    ...
    def load_schedule(filename):
        try:
            ...
            for task in data['schedule']:
                ...
                modes = []
                if task.get('baseband_enabled'): modes.append("BB")
                if task.get('spec_enabled'): modes.append(f"Spec({task.get('spec_mode')})")
                if task.get('psr_enabled'): modes.append(f"PSR({task.get('psr_mode')})")
    ...
    async def run_observation():
        ...
        if global_state.running: 
            ui.notify("Observation already running!", type='warning')
            return
            
        if not local_state['schedule_data']: 
            ui.notify("Load a schedule first", type='warning')
            return

        valid_tasks = [t for t in local_state['schedule_data']['schedule'] if not t.get('_skipped')]
        if not valid_tasks:
            ui.notify("No tasks enabled", type='warning')
            return
        ...
        try:
            ...
            with open(DRIVER_LOG_FILE, 'a') as f:
                f.write(f"\n\n--- NEW SESSION START: {json.dumps(Path.cwd().name)} ---\n")

            temp_file = obs_utils.SCHEDULE_DIR / ".active_run.json"
            original_ver = local_state['schedule_data'].get('version', 'v0.1.0')
            clean_data = {"version": original_ver, "schedule": []}
            for t in valid_tasks:
                copy_t = t.copy()
                for k in ['_skipped', 'ant_display', 'backend_display']: 
                    if k in copy_t: del copy_t[k]
                clean_data['schedule'].append(copy_t)

            with open(temp_file, 'w') as f: json.dump(clean_data, f, indent=4)

            log_file_handle = open(DRIVER_LOG_FILE, 'a')
            global_state.process = subprocess.Popen(
                [str(obs_utils.DRIVER_SCRIPT), str(temp_file)],
    ...
    def stop_observation():
        try:
            subprocess.run(["pkill", "-f", "run_auto_obs.sh"])
            with open(DRIVER_LOG_FILE, 'a') as f: f.write("\n>>> ABORT SIGNAL SENT (pkill) <<<\n")
    ...
    with ui.tab_panels(tabs, value=tab_sched).classes('w-full h-full'):
        with ui.tab_panel(tab_sched):
            with ui.row().classes('w-full items-center gap-4 mb-4'):
                ...
                update_status_ui() # Check status immediately on load
            ...
            schedule_table.add_slot('body', r'''
                <q-tr :props="props" :class="props.row._skipped ? 'bg-gray-200 text-gray-400' : ''">
                    ...
                    <q-td key="skip" :props="props">
                        <q-btn :icon="props.row._skipped ? 'undo' : 'delete'" :color="props.row._skipped ? 'grey' : 'negative'" flat dense round @click="$parent.$emit('toggle', props.row.source)">
            ...
            local_reader = LogReader(DRIVER_LOG_FILE, driver_log)
