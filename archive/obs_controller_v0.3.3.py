...
class GlobalState:
    def __init__(self):
        ...
        # Track task status: { 1: 'RUNNING', 2: 'DONE' }
        self.task_statuses = {} 

global_state = GlobalState()

# --- HELPER 1: Log Parsing & Streaming ---
class LogReader:
    def __init__(self, filepath, ui_log_element, parse_status=False, status_callback=None):
        ...
        # Regex patterns to catch Driver events
    ...
    def read(self):
        ...
        try:
            ...
            # Handle log rotation or fresh start
            if self.file is None or (self.inode and self.inode != current_inode):
                if self.file: self.file.close()
                self.file = open(self.filepath, 'r')
                self.inode = current_inode
                # If it's the master log, jump near end to avoid huge dumps
            ...
            if lines: 
                ...
                # Parse for Status Updates (Only for Driver Log)
            ...
            try:
                ...
                while self.process and self.process.returncode is None:
                    ...
                    if line: 
                        ...
                        self.ui_log.run_method('scrollToBottom') # Auto-Scroll
                    else: break
...
def main_page():
    ...
    # Store local copy of schedule for rendering
    ...
    def update_table():
        """Re-renders the table based on Global State (Status) and Local State (Data)."""
        ...
        for i, task in enumerate(local_state['schedule_data']['schedule']):
            # Display ID is 1-based (matches Driver logs)
            task_id = i + 1
            ...
            # Disable actions if global running OR task is done
    ...
    def update_status_ui():
        ...
        # Refresh table to update disabled states
        update_table()
    ...
    with ui.header().classes('bg-slate-800 text-white'):
        ...
        ui.timer(1.0, update_status_ui) # Check status every second
    ...
    with ui.tab_panels(tabs, value=tab_sched).classes('w-full h-full'):
        with ui.tab_panel(tab_sched):
            ...
            cols = [
                ...
                {'name': 'backends', 'label': 'Backends', 'field': 'backend_display', 'align': 'left'}, # NEW COLUMN
            ...
            # Dynamic Row Coloring & Disabled Action
