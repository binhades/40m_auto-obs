...
def main_page():
    ...
    def validate_and_update_ui(data, filename):
        ...
        # Pattern matches "Task 2: Invalid RA..." -> Index 1
        if not is_valid:
        ...
        for idx, task in enumerate(data.get("schedule", [])):
            ...
            if is_valid:
                ...
                if idx == error_task_index:
                    # This is the culprit row
                    task['_status'] = "ERROR"
                    # Strip the "Task X: " prefix for cleaner display
                    clean_msg = re.sub(r"^Task \d+: ", "", message)
                    task['_msg'] = clean_msg
                    task['_class'] = "bg-red-100 text-red-900 font-bold"
                else:
                    # Neutral state for other rows when file is invalid
    ...
    def handle_upload(e):
        try:
            ...
            # --- ROBUST FILE READING STRATEGY ---
            # Strategy A: Check 'file' attribute (Older NiceGUI / Variants)
            if hasattr(e, 'file'):
                f = e.file
                # Check if it has a read method
                if hasattr(f, 'read'):
                    content_bytes = f.read()
                    # Handle async read if necessary (unlikely here but safe)
                    if hasattr(content_bytes, '__await__'):
                        ui.notify("Async file read not supported in this synchronous handler", type='negative')
                        return
                    content_str = content_bytes.decode('utf-8')
                elif hasattr(f, '_data'): # Some wrappers store raw bytes in _data
                    content_str = f._data.decode('utf-8')
                else:
                    # Maybe it's just raw bytes/string already?
                    content_str = str(f)

            # Strategy B: Check 'content' attribute (Newer NiceGUI)
            elif hasattr(e, 'content'):
                content_bytes = e.content.read()
                content_str = content_bytes.decode('utf-8')
            
            else:
                raise ValueError("Could not find file content in upload event (no .file or .content)")

            # Parse & Verify
            if not content_str: raise ValueError("File was empty")
    ...
    def save_to_server():
        ...
        try:
            ...
            # Clean display fields before saving
            ...
            save_btn.disable() # Prevent double submit
    ...
    with ui.card().classes('w-full max-w-7xl mx-auto p-6 mt-10'):
        ...
        # 2. Preview Container (Visible after upload)
        with ui.column().classes('w-full gap-4') as preview_container:
            ...
            # Custom Body Slot for Row Coloring
