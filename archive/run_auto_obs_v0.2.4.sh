...
# Version: v0.2.4
# Updates: Fixed SSH Command Construction & Syntax Errors
# ==================================================================
...
if [ "$(get_mm_version "$DRIVER_VERSION")" != "$(get_mm_version "$json_ver")" ]; then
...
for (( i=0; i<$task_count; i++ )); do
    ...
    # 5. LAUNCH WORKERS (FIXED COMMAND CONSTRUCTION)
    backends_flag=""
    ...
    launch_worker() {
        ...
        # FIX: Robust Append Logic using standard checks and concatenation
        if [ -n "$spec_args" ]; then 
