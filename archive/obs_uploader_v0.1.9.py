...
VERSION = "v0.1.9 (Unique IDs & UI Fixes)"
...
def main_page():
    ...
    def reset_state():
        ...
        # Reset colors strictly
        status_card.classes(remove='bg-green-100 bg-orange-100 bg-red-100')
        status_icon.name = ''
        status_icon.classes(remove='text-green-500 text-orange-600 text-red-600')
    ...
    def delete_task(row_uuid):
        ...
        # Filter based on the hidden UUID, not the source name
        ...
        if not state['data']["schedule"]:
            ...
            status_icon.classes('text-orange-600', remove='text-green-500 text-red-600')
        ...
        try:
            ...
            if is_valid:
                ...
                for task in data["schedule"]:
                    # Assign a Temporary Unique ID for the UI logic
                    task['_ui_id'] = str(uuid.uuid4())
                ...
                # --- COLOR FIX: Explicitly remove conflicting classes ---
                if "Warnings" in message:
                    ...
                    # Use a brighter green (500) and ensure red is gone
                    status_icon.classes('text-green-500', remove='text-orange-600 text-red-600')
                ...
                status_icon.classes('text-red-600', remove='text-green-500 text-orange-600')
    ...
    def save_to_server():
        ...
        try:
            ...
            # Clean display fields AND the temporary ID before saving
            data_to_save = {"version": state['data'].get("version", "v1"), "schedule": []}
            for task in state['data']["schedule"]:
                clean = task.copy()
                for k in ['ant_display', 'backend_display', '_ui_id']: 
    ...
    with ui.card().classes('w-full max-w-7xl mx-auto p-6 mt-10'):
        ...
        with ui.column().classes('w-full gap-4') as preview_container:
            ...
            # Use the new Unique ID as the row key
