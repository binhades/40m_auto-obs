...
VERSION = "v0.2.5 (Columns Restored)"
...
def main_page():
    ...
    def validate_and_update_ui(data, filename):
        ...
        """
        # Safety Check: If UI isn't built yet, stop.
    ...
    # --- ASYNC HANDLER FOR NICEGUI 3.x ---
    async def handle_upload(e):
        try:
            content_str = ""
            filename = getattr(e, 'name', 'unknown.json')

            # NiceGUI 3.x: content is an async-readable object
            if hasattr(e, 'content'):
                # We MUST await the read
    ...
    with ui.card().classes('w-full max-w-7xl mx-auto p-6 mt-10'):
        ...
        with ui.column().classes('w-full gap-4') as preview_container:
            ...
            # --- TABLE DEFINITION UPDATED: Added duration & mode back in ---
            columns = [
