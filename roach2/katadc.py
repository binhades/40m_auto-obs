...
def iic_write_register(fpga, katadc_n, dev_addr, reg_addr, reg_value):
    ...
    # Block Fifo
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, 0x00, 0x01), offset=12)
    # Write IIC control byte
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | START | LOCK, (dev_addr << 1) | IIC_WR), offset=0x0)
    # Write IIC register address
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | LOCK, reg_addr), offset=0x0)
    # Write IIC register value
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | STOP, reg_value), offset=0x0)
    # Unblock Fifo
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, 0x00, 0x00), offset=12)
...
def iic_read_register(fpga,katadc_n, dev_addr, reg_addr):
    ...
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | LOCK, reg_addr), offset=0x0)
    # Send repeated START
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | START | LOCK, (dev_addr << 1) | IIC_RD), offset=0x0)
    # Fetch IIC register value
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, RD | STOP, 0), offset=0x0)
    # Unblock Fifo
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, 0x00, 0x00), offset=12)
    time.sleep(0.1)
...
def _eeprom_read(fpga,katadc_n,n_bytes,offset=0):
    ...
    rv=[]
    #flush the fifos:
    fpga.blindwrite(iic_controller, struct.pack('>4B', 0xff, 0xff, 0xff, 0xff), offset=0x8)
    ...
    while n_bytes_remaining > 0:
        ...
        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | START | LOCK, (dev_addr << 1) | IIC_WR), offset=0x0)
        # Write IIC register address
        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | LOCK, reg_addr+len(rv)), offset=0x0)
        # Send repeated START
        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x00, 0x00, WR | START | LOCK, (dev_addr << 1) | IIC_RD), offset=0x0)
        # Fetch IIC register value
        for i in range(min(31,n_bytes_remaining-1)):
            fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00, RD, 0), offset=0x0)
            #print '%4X'%struct.unpack('>L',fpga.read(iic_controller,4,0x8))
        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00, RD | STOP, 0), offset=0x0)
        ...
        # Unblock Fifo
        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00,0x00,0x00), offset=12)
...
#    iic_controller='iic_adc%i'%katadc_n
#    # Block Fifo
#    fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00,0x00,0x01), offset=12)
#    for n,c in enumerate(raw_data):
#        # Write IIC control byte
#        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00, WR | START | LOCK, (dev_addr << 1) | IIC_WR), offset=0x0)
#        # Write IIC register address
#        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00, WR | LOCK, n+start_addr), offset=0x0)
#        # Write IIC register value
#        fpga.blindwrite(iic_controller, struct.pack('>4B', 0x0,0x00, WR | STOP, c), offset=0x0)
#    #check for OP buffer overflow:
#    #   Bit[0] RXFIFO empty flag
#    #   Bit[1] RXFIFO full flag
#    #   Bit[2] RXFIFO overflow error latch
...
def set_noninterleaved(fpga,katadc_n,dlf=True):
    ...
    reset(fpga,katadc_n,reset=False)
    #fpga.blindwrite('kat_adc_controller', struct.pack('>4B', 0x03,0xff,0x09,0x01), offset=0x4+katadc_n*(0x04))
...
def reset(fpga,katadc_n,reset=False):
    ...
    if not katadc_n in [0,1]: raise RuntimeError("katadc_n must be 0 or 1. Please select your ZDok port.")
    fpga.blindwrite('kat_adc_controller', struct.pack('>4B', 0x00, 0x00, 0x00, reset*(0x10<<katadc_n)))
...
def cal_now(fpga,katadc_n):
    ...
    time.sleep(1)
    spi_write_register(fpga,katadc_n,0x0,0x7fff)
...
def rf_fe_set(fpga,katadc_n,input_sel,gain=0,enabled=True):
    ...
    # ZHY 2018-03-15: enable output gain value, ref. gpio_header_set() comments
    #                 this value is initialized to 0xff for 'I' input, 0 for 'Q' input
    iic_write_register(fpga,katadc_n,0x20+pol,6,0x00)
...
def gpio_header_set(fpga,katadc_n,input_sel):
    ...
    #P10 cal run (for ADC) on 0x20
    #P10 cal line (for ADC) on 0x21
    iic_write_register(fpga, katadc_n, 0x20+pol, 2, 0xC0 + gain) # MSB = switch, load_enable (gain), 6 bits = gain
    iic_write_register(fpga, katadc_n, 0x20+pol, 3, 0xFF) # write stuff in here for later comparison
    iic_write_register(fpga, katadc_n, 0x20+pol, 4, 0x00) # output inversion register for register 2 (here not inverted)
    iic_write_register(fpga, katadc_n, 0x20+pol, 5, 0x00) # output inversion register for register 3
    iic_write_register(fpga, katadc_n, 0x20+pol, 6, 0x00) # output enable (active low) for byte 2
    iic_write_register(fpga, katadc_n, 0x20+pol, 7, 0xFF) # output enable (active low) for byte 3

# def i2c_cal(fpga, katadc_n):
...
def chip_init(fpga, katadc_n, above_300mhz = True, interleaved = False):
    """Initialize ADC08D1520 Extended Control Mode configration registers"""
    if not katadc_n in [0,1]: raise RuntimeError("katadc_n must be 0 or 1. Please select your ZDok port.")
    ...
    for i in range(len(addr)):
        print('Setting ADC register %04Xh to 0x%04X for ZDOK%d' % (addr[i], val[i], katadc_n))
        # Program both ZDOKs (this could be made smarter if needed).
        spi_write_register(fpga, katadc_n, addr[i], val[i])
